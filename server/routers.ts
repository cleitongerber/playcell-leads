...
import {
  ...
  getAuditLogs,
  getDashboardFilters,
  getDashboardStatsScoped,
  getLeadActivities,
  getLeadById,
  getLeadTreatmentExport,
...
export const appRouter = router({
  ...
  leads: router({
      ...
      .input(
        z
          .object({
            ...
            view: z.enum(["available", "mine", "all"]).optional(),
          })
          .optional()
      )
      .query(({ ctx, input }) => getVisibleLeads(ctx.user, input ?? {})),
    listPage: protectedProcedure
      ...
      .query(({ ctx, input }) => getVisibleLeadsPage(ctx.user, input)),
    ...
    assume: protectedProcedure
      ...
      .mutation(({ ctx, input }) => assumeLead(input.id, ctx.user)),
    ...
    import: adminProcedure
      .input(
        z.object({
          ...
          targetPdvId: z.number().int().positive().optional(),
          useSpreadsheetPdv: z.boolean().optional(),
          rows: z.array(leadInput).min(1).max(10000),
        })
      )
      .mutation(({ ctx, input }) =>
        importLeadRows(
    ...
    dashboard: protectedProcedure
      ...
      .query(({ ctx, input }) =>
        getDashboardStatsScoped(ctx.user, {
          ...input,
          from: input?.from ? new Date(input.from) : undefined,
          to: input?.to ? new Date(input.to) : undefined,
        })
      ),
    dashboardFilters: protectedProcedure.query(({ ctx }) =>
      getDashboardFilters(ctx.user)
    ),
    productivity: protectedProcedure
      ...
      .query(({ ctx, input }) => getProductivityReport(ctx.user, input ?? {})),
    pending: protectedProcedure.query(({ ctx }) => getPendingLeads(ctx.user)),
    followUps: protectedProcedure.query(({ ctx }) =>
      getScheduledFollowUps(ctx.user)
    ),
    exportTreatments: protectedProcedure
      .input(
        z
          .object({
            pdvId: z.number().int().positive().optional(),
      ...
      .query(({ ctx, input }) => getLeadTreatmentExport(ctx.user, input ?? {})),
  }),
  team: router({
    ...
    assign: adminProcedure
      ...
      .mutation(({ input }) => assignSellerToStore(input)),
  }),
  pdvs: router({
    list: protectedProcedure
      ...
      .query(({ ctx, input }) =>
        ctx.user.role === "admin"
          ? listPdvs(input?.includeInactive)
          : listPdvs(false)
      ),
    save: adminProcedure
      .input(
        z.object({
          ...
          leadTarget: z.number().int().nonnegative().optional(),
          conversionTarget: z.number().int().nonnegative().optional(),
          isActive: z.boolean().optional(),
        })
      )
      .mutation(({ ctx, input }) => savePdv(input, ctx.user.id)),
    setActive: adminProcedure
      .input(
        z.object({ id: z.number().int().positive(), isActive: z.boolean() })
      )
      .mutation(({ ctx, input }) =>
        setPdvActive(input.id, input.isActive, ctx.user.id)
      ),
  }),
  campaigns: router({
    list: protectedProcedure
      .input(z.object({ includeInactive: z.boolean().optional() }).optional())
      .query(({ ctx, input }) =>
        listCampaigns(ctx.user, input?.includeInactive)
      ),
    save: adminProcedure
      .input(
        z.object({
      ...
      .mutation(({ ctx, input }) => saveCampaign(input, ctx.user.id)),
    ...
    setFrozen: adminProcedure
      ...
      .mutation(({ ctx, input }) =>
        setCampaignFrozen(input.id, input.isFrozen, ctx.user.id)
      ),
  }),
  users: router({
    ...
    create: adminProcedure
      .input(
        z.object({
          ...
          pdvIds: z.array(z.number().int().positive()).max(50),
        })
      )
      .mutation(async ({ ctx, input }) =>
        createManagedUser(
    ...
    deactivate: adminProcedure
      ...
      .mutation(({ ctx, input }) =>
        deactivateManagedUser(input.userId, ctx.user.id)
      ),
    updateAccess: adminProcedure
      ...
      .mutation(({ ctx, input }) => updateUserAccess(input, ctx.user.id)),
  }),
  audit: router({
    list: adminProcedure
      .input(
