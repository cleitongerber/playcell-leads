...
export default function ImportLeads() {
  ...
  const campaign = (campaignsQuery.data ?? []).find((item) => item.id === Number(campaignId));
  const pdvs = (campaign?.pdvs ?? []).filter((pdv) => pdv.isActive);
  useEffect(() => {
    if (!campaignId && campaignsQuery.data?.[0]) setCampaignId(String(campaignsQuery.data[0].id));
